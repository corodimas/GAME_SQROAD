...
bool LeaderBoard::loadFile() {
    ...
    while (std::getline(file_read, read_string)) {
        ...
        if (is_read_name) {
            ...
            Score2 score(read_name, read_score_int);
...
bool LeaderBoard::addEntry(Score2 player) {

    return this->addScoreEntry(player);
}


bool LeaderBoard::addEntry(std::string name, unsigned long score) {

    return this->addScoreEntry(Score2(name, score));
}

bool LeaderBoard::addScoreEntry(Score2 player) {
    ...
    void LeaderBoard::printEntry(unsigned long max_entry) {
...
std::vector <Score2> LeaderBoard::get() {

    return this->score_list;
}


void LeaderBoard::set(std::vector <Score2> score_list) {

    this->score_list = score_list;
}


unsigned long LeaderBoard::clear() {
...
void LeaderBoard::removeEntry(unsigned long index) {
...
Score2 LeaderBoard::getEntry(unsigned long index) 
{
    if (index >= this->score_list.size()) {
...
std::string LeaderBoard::getFileName() {
